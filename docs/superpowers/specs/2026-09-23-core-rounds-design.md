...
- **The assistant: 手順を見る on every question.** A 手順を見る button sits under the prompt in both bead and keypad mode, in rounds and in 基礎. It opens a step panel: the explanation lines with ◀ ▶ and 最初から. The soroban shows the steps, drawn solid at any fade level, and the answer controls wait until とじる, which brings back the learner's own beads and typed answer.
