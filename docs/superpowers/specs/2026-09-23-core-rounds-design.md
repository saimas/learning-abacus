...
- **Before an answer:** a small 手順を見る button (`steps-open`) where the step lines appear once it is opened. The owner (2026-09-24) asked for this, to keep it consistent: it first sat under the prompt while the steps showed at the bottom of the screen. In bead mode it sits below the soroban, the operand board and the hint, at the top of the flexible space the lines take, with もどす / こたえる below it. The button adds nothing to that space's share of the height, so it does not move the soroban (opening the panel does; see the bead-mode layout below), and the space's share is the one the scroll above had when it had to show the prompt and the button both, so the button fits wherever the scroll showed it before. Where the spare height still leaves the space shorter than the button, the space scrolls (a ScrollView with the same outer `{ flex: 1 }`), so a small scroll reaches the button rather than it spilling over もどす / こたえる. (No minimum height on the space: Yoga counts one before the share-out, like a flex basis, which would move the soroban up by half of it.) In keypad mode it sits in the scroll after the prompt, the F0 demonstration line and the operand board. The F0 demonstration line stays under the prompt. It opens the panel with `showAnswer: false` and marks the question `assisted`. While open: the soroban shows `stepper.soroban ?? shownBeads`, which is the start as the panel opens (§3), drawn solid at any fade level, takes no taps, and the answer controls (bead mode's もどす / こたえる, keypad mode's pad) are replaced by the panel's とじる. とじる clears the stepper and closes the panel; the learner's beads and typed answer are untouched throughout.
...
- **Bead mode, with the panel open** (before an answer or in a miss's review): the step controls sit under the soroban and the operand board, and the step lines fill the space below them to the bottom, scrolling on their own (in review つぎへ keeps its row below them). The top scroll, which holds the prompt, is then only as tall as the prompt (`flexGrow: 0`, and `flexShrink: 1` so it still gives way and scrolls on a very short screen), so the soroban and the board move up under it and the lines take all the height that is left. The owner (2026-09-24, on a 375 × 667 phone) found the space between the prompt and the soroban standing empty while the lines were cut off at the bottom, and asked to spread the space so it is put to use. This replaces the earlier ruling that the top scroll kept its flex share so the soroban did not move as the panel opened: the owner would rather have the room. With the panel closed the layout is as before (the top scroll and the space under the hint share the spare height). Keypad mode is unchanged.
