...
- **Step lines** (`ProblemCorrectionCard`):
  - quotient group: `quotientLine(q, partial, d0, guess, split, remainderZero)`: the guess by 九九, why it was lowered if it was too big, then where q goes, e.g. "16÷3で見当をつけると5。5だと引ききれないので4にする。商4を頭の1つ左に立てる", "4÷3で見当をつけると1。商1を頭の2つ左に立てる". (en: "Estimate 16 ÷ 3 = 5. 5 is too big to take away, so use 4. Place 4 one rod left of the head.") Four cases read differently, so the line never shows wrong arithmetic or an odd sum:
...
- `problem.test.ts`:
  ...
  - the guess: those three problems' `partial` and `guess`; exhaustively, `q ≤ guess ≤ 9`, `partial ≤ 99`, `guess = q` for a 1-digit divisor, and `remainderZero` exactly when the rods below the quotient's read 0.
