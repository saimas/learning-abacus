...
Expected: FAIL — `Cannot find module './ja'` until Steps 1-2 are saved; once they are, the run should go green. If any `coaching` assertion fails, fix the catalog, not the test: the expected strings are the spec's §5 and §6 tables.
